...
impl Settings {
    ...
    fn settings_mouse_utils<'a>(&self, tr: &'a translations::Tr, ui: Ui) -> Element<'a, Message> {
        ...
        let activation_str = match cfg.find_my_mouse_activation {
            FindMyMouseActivation::LeftCtrlTwice => "left-ctrl-twice",
            FindMyMouseActivation::RightCtrlTwice => "right-ctrl-twice",
            FindMyMouseActivation::ShakeMouse => "shake-mouse",
            FindMyMouseActivation::CustomShortcut => "custom-shortcut",
        };

        let hl_mode_str = match cfg.highlighter_mode {
            HighlightMode::CircleHighlight => "circle",
            HighlightMode::Spotlight => "spotlight",
        };

        let orient_str = match cfg.crosshair_orientation {
            CrosshairOrientation::Horizontal => "horizontal",
            CrosshairOrientation::Vertical => "vertical",
            CrosshairOrientation::Both => "both",
        };

        let heading = |s: &'a str| -> Element<'a, Message> {
            text(s)
                .size(ui.sz(14.0))
                .font(ui.font())
                .color(ui.heading())
        ...
        if cfg.find_my_mouse {
            ...
            // Game mode toggle
            col = col.push(pref_toggle(
                tr.ms_fmm_game_mode,
                tr.ms_fmm_game_mode_desc,
                cfg.find_my_mouse_game_mode,
                Message::ToggleFindMyMouseGameMode,
                ui,
            ));
        ...
        if cfg.crosshair {
            col = col
                ...
                .push(heading(tr.ms_ch_orientation))
                .push(
                    container(
                        row![
                ...
                .push(pref_toggle(
                    tr.ms_ch_auto_hide,
                    tr.ms_ch_auto_hide_desc,
                    cfg.crosshair_auto_hide,
                    Message::ToggleCrosshairAutoHide,
                    ui,
                ))
                .push(pref_toggle(
                    tr.ms_ch_fixed_length,
                    tr.ms_ch_fixed_length_desc,
                    cfg.crosshair_fixed_length,
                    Message::ToggleCrosshairFixedLength,
                    ui,
                ));
